...
void explainList()
{
    ...
    ls.push_fronts(5); //{5,2,4}
    ls.emplace_front(); //{2,4}

    // Another function same as vector
...
void explainMultiSet() 
{
    ...
    int cnt = ms.count(1);
    // only a single one erased ms.erase(ms.find(1));
...
id explainMap() 
{
    ...
    for(auto it: mpp) 
    {
        ...
        cout << *(it).second;
        auto it = mpp.find(5);
        // This is the syntax auto it mpp.lower_bound(2);
...
pid explainMultimap() 
{
    ...
    oid explainUnorderedMap() {
    // same as set and unordered_Set difference.
    wool comp(pair<int, int>p1, pair<int, int>p2) {
    if (p1.second < p2.second) 
    {
     return true;
    } else if(pl.second == p2.second)
    {
